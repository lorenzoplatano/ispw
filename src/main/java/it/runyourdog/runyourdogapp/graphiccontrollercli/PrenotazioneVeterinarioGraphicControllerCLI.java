...
public class PrenotazioneVeterinarioGraphicControllerCLI extends ProfiloPadroneGraphicControllerCLI{

    public PrenotazioneVeterinarioGraphicControllerCLI(UserBean loggedUser, ProfiloPadroneBean padrone) {
        super(loggedUser);
        this.profilo = padrone;
        this.controller = new PrenotazioneVeterinarioController();
    }

    protected PrenotazioneVeterinarioController controller;
    ...
    public void showMenu() {
        Printer.printf("*---- PRENOTAZIONE VETERINARIO ----*");

        int choice;

        while (true) {

            Printer.printf("1) Inizia prenotazione veterinario");
            Printer.printf("2) Torna al profilo");
            Printer.printf("3) Gestisci prenotazioni");
            Printer.printf("4) Prenota Dogsitter");
            ...
            try {
                switch (choice) {
                    ...
                    case 4 -> new PrenotazioneDogsitterGraphicControllerCLI(loggedUser, profilo).start();
    ...
    private void startBooking() {
        try {
            PrenotazioneBean request = findVet();
            if (request == null) {
                showMenu();
            }
    ...
    private PrenotazioneBean findVet() throws InvalidInputException {
        while (true) {
            ...
            List<ProfiloVeterinarioBean> candidates = List.of();
            try {
                candidates = controller.cercaVeterinario(bean);
            } catch (DAOException e) {
            ...
            if (candidates.isEmpty()) {
                Printer.printf("Nessun veterinario disponibile. Torno al menu principale,\n");
                return null;
            }


            PrenotazioneBean result = chooseVet(bean, candidates);
            if (result == null) {
                return null;
            }
            return result;
    ...
    private boolean collectDetails(PrenotazioneBean bean) {
        ...
        while (true) {
            try {
                Printer.printf("Inserisci la città in cui cerchi il veterinario:");
    ...
    private PrenotazioneBean chooseVet(PrenotazioneBean bean, List<ProfiloVeterinarioBean> list) {
        Printer.printf("Veterinari trovati:");
        for (int i = 0; i < list.size(); i++) {
            ProfiloVeterinarioBean d = list.get(i);
            Printer.printf(String.format(
                    "%d) %s, %d anni, %s, Tel: %s, Email: %s, Indirizzo: %s",
                    i + 1, d.getNome(), d.getEta(), d.getGenere(), d.getTelefono(), d.getEmail(), d.getIndirizzo()
            ));
        }
        while (true) {
            Printer.printf("Seleziona il numero del veterinari desiderato (0 per annullare):");
            try {
